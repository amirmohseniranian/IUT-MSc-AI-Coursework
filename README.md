...
1. **Small training set.** The dataset contains only 40 labeled samples, so the reported training metrics are descriptive of this assignment rather than evidence of generalization performance.
2. **No held-out evaluation.** The script reports training accuracy; it does not create a validation or test split.
3. **Perceptron order dependence.** The observed convergence epochs depend on the deterministic presentation order and initialization.
4. **Unregularized logistic optimization.** There is no L1/L2 regularization, line search, or second-order optimizer. The fixed learning rate and iteration budget are implementation choices rather than assignment-independent optimal settings.
5. **OVR score interpretation.** The four sigmoid outputs are separately fitted binary scores, not a calibrated joint four-class probability vector.
6. **OVO tie handling.** `numpy.argmax` selects the first class in the deterministic class ordering when vote totals tie, while a separate Boolean flag records the tie. Thus, the reported predicted class at a tie should not be interpreted as uniquely supported by the voting system.
7. **Hard-coded assignment data.** The source is purpose-built for the supplied training table rather than a generic dataset-loading interface.
8. **Dependency versions are not pinned.** The script prints library versions for auditability but does not include a lockfile or requirements file.

## Future Work

A research-oriented extension could add an explicit configuration layer for optimizer hyperparameters, regularized logistic regression, multiple randomized Perceptron presentation orders, and a held-out evaluation protocol. Calibration diagnostics could also be added for the OVR scores, along with confusion matrices and uncertainty summaries.

For reproducibility, the repository could also include a `requirements.txt` or `pyproject.toml`, record the Python version, and separate the dataset definition, model implementations, experiments, and visualization into importable modules with a small automated test suite.


<!-- Source basis: :contentReference[oaicite:6]{index=6} -->
